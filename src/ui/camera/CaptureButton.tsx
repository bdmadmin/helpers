...
import Reanimated, {cancelAnimation, Easing, Extrapolate, interpolate, useAnimatedStyle, withSpring, withTiming, useAnimatedGestureHandler, useSharedValue, withRepeat} from 'react-native-reanimated';
import type {Camera, PhotoFile, TakePhotoOptions, TakeSnapshotOptions, VideoFile} from 'react-native-vision-camera';
import {CAPTURE_BUTTON_SIZE, SCREEN_HEIGHT, SCREEN_WIDTH} from './../../config/constant';
...
interface Props extends ViewProps {
  ...
  clickPictureOnly: boolean;
}

const _CaptureButton: React.FC<Props> = ({camera, onMediaCaptured, minZoom, maxZoom, cameraZoom, flash, enabled, setIsPressingButton, style, clickPictureOnly, ...props}): React.ReactElement => {
    ...
    try {
      ...
      camera.current.startRecording({
        ...
        onRecordingFinished: video => {
          console.log(`Recording successfully finished!`, JSON.stringify(video));
  ...
  const onHandlerStateChanged = useCallback(
    ...
    [isPressingButton, recordingProgress, setIsPressingButton, startRecording, stopRecording, takePhoto, clickPictureOnly],
  );
  ...
  return (
    <TapGestureHandler
      ...
      <Reanimated.View {...props} style={[buttonStyle, style]}>
        <PanGestureHandler enabled={enabled} ref={panHandler} failOffsetX={PAN_GESTURE_HANDLER_FAIL_X} activeOffsetY={PAN_GESTURE_HANDLER_ACTIVE_Y} onGestureEvent={onPanGestureEvent} simultaneousHandlers={tapHandler}>
